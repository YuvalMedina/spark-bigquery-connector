...
public class SchemaConverterTest {
    ...
    public void testFieldHasDescriptionBigQueryToSpark() throws Exception {
        Schema bqSchema = Schema.of(
                Field.newBuilder("name", LegacySQLTypeName.STRING)
                        ...
                        .build());
        StructType expected = new StructType()
                .add(new StructField("name", DataTypes.StringType, true,
    ...
    public void testMaximumNestingDepthError() throws Exception {
        ...
        for(int i = 0; i < MAX_BIGQUERY_NESTED_DEPTH+1; i++){
            StructType outer = new StructType()
    ...
    public final StructField SPARK_INTEGER_FIELD = new StructField("Number", DataTypes.IntegerType,
            true, Metadata.empty());
    public final StructField SPARK_STRING_FIELD = new StructField("String", DataTypes.StringType,
            false, Metadata.empty());
    public final StructField SPARK_NESTED_STRUCT_FIELD = new StructField("Struct", MY_STRUCT,
            true, Metadata.empty());
    ...
    public final StructType BIG_SPARK_SCHEMA2 = new StructType()
            ...
            .add(new StructField("binary_arr", DataTypes.createArrayType(DataTypes.BinaryType, true),
                    true, Metadata.empty()))
            .add(new StructField("float", DataTypes.DoubleType, true, Metadata.empty()))
            .add(new StructField("numeric", DataTypes.createDecimalType(38, 9),
                    true, Metadata.empty()))
            .add(new StructField("date", DataTypes.DateType, true, Metadata.empty()))
            .add(new StructField("times", new StructType()
                    ...
                    .add(new StructField("datetime", DataTypes.StringType, true, Metadata.empty())),
                    true, Metadata.empty()));

    public final Schema BIG_BIGQUERY_SCHEMA2 = Schema.of(
            Field.of("foo", LegacySQLTypeName.STRING),
            Field.of("bar", LegacySQLTypeName.INTEGER),
            Field.newBuilder("required", LegacySQLTypeName.BOOLEAN).setMode(Field.Mode.REQUIRED).build(),
            Field.newBuilder("binary_arr", LegacySQLTypeName.BYTES).setMode(Field.Mode.REPEATED).build(),
            Field.of("float", LegacySQLTypeName.FLOAT),
            Field.of("numeric", LegacySQLTypeName.NUMERIC),
            Field.of("date", LegacySQLTypeName.DATE),
            Field.of("times", LegacySQLTypeName.RECORD,
